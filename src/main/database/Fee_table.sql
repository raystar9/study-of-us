...
SELECT FM_ID, FM_M_INDEX, FM_F_ID, FM_FEE, FM_NOTE FROM FeeMember;
INSERT INTO FeeMember(FM_ID, FM_M_INDEX, FM_F_ID, FM_FEE, FM_NOTE) VALUES (feemember_id.NEXTVAL, ?, ?, ?, ?);
UPDATE FeeMember SET FM_M_INDEX=?, FM_F_ID=?, FM_FEE=?, FM_NOTE=? WHERE FM_ID=?;
...
INSERT INTO FeeExpense(FE_ID, FE_F_ID, FE_EXPENSE, FE_EXPENSEFEE) VALUES (?, ?, ?, ?);
UPDATE FeeExpense SET FE_F_ID=?, FE_EXPENSE=?, FE_EXPENSEFEE=? WHERE FE_ID=?;
