...
SELECT S_INDEX, S_NAME, S_C_ID, S_MT_INDEX, S_START, S_END, S_PLOPIENUM, S_DAY, S_TIME, S_EXPLAIN, S_PREPARED, S_EFFECTIVE, S_PLACE FROM Study;
INSERT INTO Study(S_INDEX, S_NAME, S_C_ID, S_MT_INDEX, S_START, S_END, S_PLOPIENUM, S_DAY, S_TIME, S_EXPLAIN, S_PREPARED, S_EFFECTIVE, S_PLACE)
	VALUES (study_index.NEXTVAL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
UPDATE Study SET S_INDEX=?, S_NAME=?, S_C_ID=?, S_MT_INDEX=?, S_START=?, S_END=?, S_PLOPIENUM=?, S_DAY=?, S_TIME=?, S_EXPLAIN=?, S_PREPARED=?, S_EFFECTIVE=?, S_PLACE=?
	WHERE S_INDEX=?;
DELETE FROM Stu                                         dy WHERE S_INDEX = ?;
